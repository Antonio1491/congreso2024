...
return $element;
     ...
     $htmlBloque .=' <img src="build/img/icon-eventos-sociales.png" alt="">';
      ...
      <script>
        ...
        if($bloque['tipo']=='Talleres')
        {
          ...
          // var_dump($bloque['id'],$fechaInicio,$bloque['inicio'],$bloque['fin'],$bloque['tipo']);
        }
      $htmlBloque .= ' </article>';
  ...
  function resumen($texto, $largor = 15, $puntos = "...")
  {
   ...
   if (count($palabras) > $largor)
    ...
    public function talleres($fecha, $inicio, $fin, $congreso = NULL){
    ...
    public function taller($congreso = NULL){
      $sql = "SELECT * FROM talleres ";
      $consulta = $this->conexion_db->query($sql);
      $array_resultados = $consulta->fetch_all(MYSQLI_ASSOC);
    ...
    {
      $sql = "SELECT *, ponencias.id as ponenciasID FROM ponencias 
      JOIN conferencias ON 
